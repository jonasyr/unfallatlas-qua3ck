...
Beyond the AI models listed per phase below, every Codex and Claude Code session on this project ran with the following tooling attached. These are not one-off, phase-specific uses — they were active in essentially every session and materially shaped how the AI tool worked (what it could see, how precisely it could edit, and how it turned a request into a plan before touching any file), so they are disclosed here once rather than repeated per row.

- **[Serena MCP](https://github.com/oraios/serena)** — a semantic, symbol-aware code-navigation and editing server. Instead of dumping whole files into context, it lets the AI look up a function/class by name, read only the symbol it needs, and edit at the symbol level. I use it because it keeps edits precise and scoped (less risk of the AI silently rewriting unrelated code) and keeps context usage low enough to work across this repo's larger notebooks and modules without truncation.
- **[Codebase Memory MCP](https://github.com/DeusData/codebase-memory-mcp)** — a graph-indexed memory of the repository (call graphs, data-flow, architecture) that the AI can query instead of re-deriving structure from scratch every session. I use it so the AI can trace how a change in one module (e.g. `src/unfallatlas/features/preprocessing.py`) actually propagates to the notebooks that import it, rather than guessing from file names.
- **[Superpowers Plugin](https://github.com/obra/superpowers)** — a library of structured workflow skills (`brainstorming`, `writing-plans`, `executing-plans`, `systematic-debugging`, among others) that govern *how* a request turns into work: clarify and design first, write an explicit reviewable implementation plan second, execute task-by-task third. I use it to keep larger builds (e.g. the full C-phase notebook) from becoming one big unreviewed AI-authored blob — every non-trivial build in this project has a corresponding design spec and/or implementation plan under `docs/superpowers/` as a paper trail.

None of these tools generate project content on their own — they change how the underlying AI model (Claude/Codex, as listed per row below) reads and edits this specific repository. Their effect on the end result is not small: which files get read, how narrowly an edit is scoped, and whether a build proceeds via an upfront plan versus ad hoc are all downstream of this tooling, even though the tools themselves never appear as an "AI tool" row in the table below.

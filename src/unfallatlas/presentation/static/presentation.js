window.UnfallatlasPresentation = (() => {
  ...
  async function loadPlotly(container) {
    ...
    const loading = (async () => {
      try {
        ...
        message.textContent =
          error instanceof Error ? error.message : "The chart could not be loaded.";
  ...
  function initRepoLink() {
    ...
    link.target = "_blank";
    link.rel = "noopener";
    link.setAttribute("aria-label", "Open GitHub repository (new tab)");
    link.innerHTML =
  ...
  function initialize() {
    ...
    document.addEventListener("click", async (event) => {
      ...
      if (action === "print") {
        announce("Preparing charts for printing.");
